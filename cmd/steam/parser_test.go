...
func TestGameDetailsToMap(t *testing.T) {
	...
	assert.Equal(t, 30, result["playtime_recent"])
	assert.Equal(t, testTime.String(), result["last_played"])
	assert.Equal(t, true, result["details_fetched"])
	assert.Equal(t, "This is a detailed description of the test game.", result["description"])
	assert.Equal(t, "Short description", result["short_desc"])
...
func TestGameDetailsToMap_MinimalData(t *testing.T) {
	...
	assert.Equal(t, 0, result["playtime_recent"])
